---
sidebar_position: 1
sidebar_label: "Expedição de nota"
title: "Expedição de nota"
---
...
A tela **Expedição de Nota** pertence ao módulo de **Expedição**, mas também pode ser alocada no módulo de **Administração**, conforme a necessidade da operação. É uma tela **parcialmente online**. Nela, o usuário pode realizar a **expedição de produtos pertencentes a uma nota**.


:::info Conexão necessária
  É uma tela parcialmente **online**. Embora os dados das **notas e produtos** sejam carregados a partir de **tabelas armazenadas na base local**, o aplicativo realiza uma **consulta à API** para verificar se a nota ainda está **disponível para expedição** antes de permitir o início do processo.
:::


## Informações da Tela

Ao tocar no card que representa o nota, a tela exibe todos os produtos vinculados, que devem passar pelo processo de expedição.

No cabeçalho da tela, no canto superior direito, há o ícone para **cancelar o carregamento da nota**.  
...
Na parte inferior da tela, são exibidos o **número da nota**, a **quantidade de produtos já carregados** e uma **barra de progresso** com a porcentagem do carregamento.  
Quando atinge **100%**, o botão **Finalizar Nota** é habilitado.
...
Ao selecionar uma nota, o sistema realiza uma **verificação de disponibilidade** para garantir que ela ainda não esteja em uso.  
Somente após essa validação a nota é **vinculada ao usuário** que iniciou o processo.
Mesmo que o aplicativo seja fechado, a nota permanece **disponível apenas para o mesmo usuário**, impedindo o acesso de outros até a finalização.

Enquanto houver uma nota em andamento, **outras notas permanecem ocultas**.

<div className="images-grid">
  <img src={nota1} alt="Card da nota selecionada" />
</div>
...
<div className="images-grid">
  ...
  <img src={nota5} alt="Sem nota selecionada" />
</div>
...
Caso o usuário feche a tela ou o aplicativo durante a expedição, ao voltar a abrir a **nota selecionada**, o sistema **retoma automaticamente o progresso do carregamento** exatamente do ponto em que parou — mantendo os produtos já conferidos marcados e o restante pendente.
:::
...
<div className="images-grid">
  <img src={nota6} alt="Tela com botão de finalizar nota habilitado" />
  <img src={nota7} alt="Tela com nota finalizada no topo" />
</div>

O **card da nota finalizada** passa a ser a identificada pela legenda **Nota fiscal finalizada**, exibida no canto superior esquerdo do card.  

<div className="images-grid-2">
  <img src={nota8} alt="Card de nota finalizada destacado" />
</div>
